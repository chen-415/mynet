...
# img_path = "/media/happy507/DataSpace1/liuyuxin/even_results/net2_results/outdoor_results/Net_results/0140_0.95_0.12.jpg"
...
def calculate_brisque_for_folder(folder_path):
    """
    计算文件夹中所有图像的 BRISQUE 分数，并输出平均值。
    参数：
        folder_path (str): 文件夹路径。
    返回：
        float: 文件夹中所有图像的平均 BRISQUE 分数。
    """
    obj = BRISQUE(url=False)
    brisque_scores = []
    total_files = 0
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            ...
            try:
                ...
                # 计算 BRISQUE 得分
                score = obj.score(img=img_ndarray)
                brisque_scores.append(score)

                print(f"Processed {total_files}: {filename} - BRISQUE Score: {score:.4f}")
    ...
    print(f"\nAverage BRISQUE Score for folder: {average_score:.4f}")
...
if __name__ == "__main__":
    ...
    try:
        calculate_brisque_for_folder(folder_path)
    except Exception as e:
        print(f"Error: {e}")
